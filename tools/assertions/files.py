...
def assert_get_file_with_incorrect_file_id_response(
    ...
    :param incorrect_file_id: Некорректный file_id.
    :param actual: Ответ от API с ошибкой валидации, который необходимо проверить.
    :raises AssertionError: Если фактический ответ не соответствует ожидаемому.
    """
    expected = ValidationErrorResponseSchema(
        details=[
            ValidationErrorSchema(
                ...
                context={"error": "invalid character: "
                                  ...
                                  f"followed by [0-9a-fA-F-], found `{incorrect_file_id[:1]}` at 1"},
                message=f"Input should be a valid UUID, invalid character: "
                        "expected an optional prefix of `urn:uuid:` "
                        f"followed by [0-9a-fA-F-], found `{incorrect_file_id[:1]}` at 1",
